...
function SkillUsagePreview({ exec }: { exec: ToolExecution }) {
  ...
  return (
    <div className="mx-3 mb-2 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
      <span className="font-semibold text-foreground/80">{tr("专业 Skill", "Professional skills", "Kỹ năng chuyên nghiệp")}</span>
      {skills.map((skill) => (
...
function ChapterContextTracePreview({ exec }: { exec: ToolExecution }) {
  ...
  return (
    <div className="mx-3 mb-3 mt-1 rounded-xl border border-border/50 bg-background/55 px-3 py-2.5 text-xs">
      <div className="font-semibold text-foreground">{tr("本轮参考依据", "Context used this turn", "Cơ sở tham chiếu lượt này")}</div>
          ...
          <details key={`${trace.chapterNumber ?? 0}:${trace.tracePath}`} className="rounded-lg border border-border/40 px-2.5 py-2">
            ...
            <div className="mt-2 space-y-1.5 text-muted-foreground">
              <div>
                ...
                {` · ${tr("可压缩", "compressible", "nén được")} ${trace.compressibleTokens ?? 0}`}
              </div>
              <div>
              ...
              <div>{tr("完整来源", "All sources", "Tất cả nguồn")}:</div>
    ...
    <div className="mt-2 space-y-1.5">
      ...
      {issues.map((issue, index) => (
        <div key={`${issue.category}:${index}`} className="rounded-lg border border-border/40 bg-background/55 px-2.5 py-2 text-[12px] leading-5 text-muted-foreground">
          ...
          {issue.suggestion && <div className="mt-0.5">{tr("建议", "Suggestion", "Gợi ý")}{tr("：", ": ", ": ")}{issue.suggestion}</div>}
        </div>
      ))}
    </div>
  );
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="text-[15px] font-semibold text-foreground">
          {details.chapterNumber ? tr(`第 ${details.chapterNumber} 章修订`, `Chapter ${details.chapterNumber} revision`, `Sửa đổi chương ${details.chapterNumber}`) : tr("章节修订", "Chapter revision", "Sửa đổi chương")}
        </div>
        <div className={`rounded-full px-2 py-0.5 text-[12px] font-semibold ${passed ? "bg-emerald-500/15 text-emerald-600" : "bg-amber-500/15 text-amber-600"}`}>
          {!details.applied
            ? tr("保留原稿", "Original kept", "Giữ nguyên bản gốc")
            : details.auditPassed
              ? tr("审稿通过", "Audit passed", "Thông qua kiểm duyệt")
...
function ChapterStateResyncPreview({ exec }: { exec: ToolExecution }) {
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="text-[15px] font-semibold text-foreground">
          {details.chapterNumber ? tr(`第 ${details.chapterNumber} 章状态已同步`, `Chapter ${details.chapterNumber} state resynced`, `Trạng thái chương ${details.chapterNumber} đã đồng bộ`) : tr("章节状态已同步", "Chapter state resynced", "Trạng thái chương đã đồng bộ")}
        </div>
        <div className={`rounded-full px-2 py-0.5 text-[12px] font-semibold ${passed ? "bg-emerald-500/15 text-emerald-600" : "bg-amber-500/15 text-amber-600"}`}>
          {passed ? tr("审稿通过", "Audit passed", "Thông qua kiểm duyệt") : tr("仍需修订", "Revision required", "Cần sửa đổi")}
        </div>
      </div>
...
function ScriptStoryboardResultPreview({ exec, onOpenFilmStudio }: { exec: ToolExecution; onOpenFilmStudio?: (projectId: string) => void }) {
  ...
  const maybeRows: Array<readonly [string, string] | null> = [
    details.specPath ? [tr("规格", "Spec", "Thông số kỹ thuật"), details.specPath] : null,
    details.storyGraphPath ? [tr("剧情图谱", "Story graph", "Biểu đồ cốt truyện"), details.storyGraphPath] : null,
    ...
    details.imagePromptsPath ? [tr("图像提示词", "Image prompts", "Gợi ý hình ảnh"), details.imagePromptsPath] : null,
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="text-[16px] leading-6 font-semibold text-primary">
          {details.kind === "script_created"
            ...
            : details.kind === "storyboard_created"
              ? tr("分镜已生成", "Storyboard generated", "Đã tạo storyboard")
        ...
        {details.kind === "interactive_film_created" && details.projectId && onOpenFilmStudio && (
          ...
          >
            {tr("打开创作向导 →", "Open creation wizard →", "Mở trình hướng dẫn tạo →")}
          </button>
        )}
      </div>
      {rows.length > 0 && (
...
function PlaySceneImagePreview({ details }: { details: PlayToolDetails }) {
  ...
  return (
    <div className="mt-3 overflow-hidden rounded-xl border border-border/40 bg-background/80">
      <img
        src={readyUrl}
        alt={tr("本幕配图", "Scene illustration", "Minh họa phân cảnh")}
      ...
      {details.turn != null && (
        <div className="border-t border-border/40 px-3 py-2.5 text-[14px] leading-6 text-muted-foreground">
          {tr(`第 ${Math.trunc(details.turn)} 幕配图`, `Scene ${Math.trunc(details.turn)} illustration`, `Minh họa phân cảnh ${Math.trunc(details.turn)}`)}
        </div>
      )}
    </div>
  );
...
function ProposedActionPreview({
  ...
  return (
    <div className="mx-3 mb-3 mt-1 rounded-xl border border-primary/25 bg-primary/5 px-4 py-3.5">
      <div className="text-[17px] leading-6 font-semibold text-foreground">{details.title ?? tr("确认执行", "Confirm action", "Xác nhận thực hiện")}</div>
      {details.summary && (
...
function PlayResultPreview({ exec }: { exec: ToolExecution }) {
  ...
  const label = details.kind === "play_world_started"
    ...
    : details.kind === "play_turn_revised"
      ? tr("互动回合已重做", "Play turn redone", "Đã làm lại lượt tương tác")
      : details.kind === "play_variant_restored"
        ? tr("已切换互动回合版本", "Switched play turn variant", "Đã chuyển phiên bản lượt tương tác")
        : tr("互动世界已推进", "Interactive world advanced", "Thế giới tương tác đã phát triển");
  return (
...
function PlayEditPreview({ exec }: { exec: ToolExecution }) {
  ...
  return (
    <div className="mx-3 mb-3 mt-1 rounded-xl border border-primary/20 bg-primary/5 px-3 py-2.5">
      <div className="text-[16px] leading-6 font-semibold text-primary">{tr("互动世界设定已更新", "Interactive world settings updated", "Cài đặt thế giới tương tác đã cập nhật")}</div>
