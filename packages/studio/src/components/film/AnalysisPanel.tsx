  ...
  return (
    <div className="border border-border rounded p-3" data-testid="validation-panel">
      <div className={`text-sm font-medium ${c.muted}`}>
        {tr("校验", "Validation", "Xác thực")}{report.ok ? "" : tr("（有阻断问题）", " (blocking issues)", " (vấn đề chặn)")}
      </div>
...
function EmotionArcChart({ arcs, c }: { arcs: EmotionArcs; c: Colors }) {
  ...
  return (
    <div data-testid="emotion-arc" className="border border-border rounded p-3">
      <div className={`text-sm font-medium mb-2 ${c.muted}`}>{tr("情感曲线", "Emotion arcs", "Đường cảm xúc")}</div>
      {displayArcs.length === 0 ? (
        <div className={`text-sm ${c.muted}`}>{tr("暂无可分析路径", "No paths to analyze", "Chưa có đường dẫn để phân tích")}</div>
      ) : (
        <>
          <svg
            width="100%"
            viewBox={`0 0 ${SVG_W} ${SVG_H}`}
            aria-label={tr("情感曲线图", "Emotion arc chart", "Biểu đồ đường cảm xúc")}
          ...
          {(overLimit || arcs.truncated) && (
            <div className={`text-xs mt-1 ${c.muted}`}>
              {overLimit && tr(`仅显示前 ${MAX_ARC_DISPLAY} 条路径`, `Showing first ${MAX_ARC_DISPLAY} paths only`, `Chỉ hiển thị ${MAX_ARC_DISPLAY} đường dẫn đầu tiên`)}
              {arcs.truncated && tr("（路径总数已超过枚举上限）", " (total paths exceed the enumeration limit)", " (tổng số đường dẫn vượt giới hạn liệt kê)")}
            </div>
          )}
        </>
      )}
...
function PathDistributionPanel({
  ...
  return (
    <div data-testid="path-distribution" className="border border-border rounded p-3">
      <div className={`text-sm font-medium mb-2 ${c.muted}`}>{tr("路径分布", "Path distribution", "Phân bố đường dẫn")}</div>

      {distribution.truncated && (
        <div className={`text-xs mb-2 ${c.muted}`}>
          {tr(`路径过多，仅统计前 ${distribution.total} 条`, `Too many paths; only the first ${distribution.total} are counted`, `Quá nhiều đường dẫn; chỉ đếm ${distribution.total} đầu tiên`)}
        </div>
      )}

      {endingEntries.length === 0 ? (
        <div className={`text-sm ${c.muted}`}>{tr("暂无路径数据", "No path data", "Chưa có dữ liệu đường dẫn")}</div>
      ) : (
      ...
      {histEntries.length > 0 && (
        <div>
          <div className={`text-xs font-medium mb-2 ${c.muted}`}>{tr("路径长度分布", "Path length distribution", "Phân bố độ dài đường dẫn")}</div>
              ...
              return (
                ...
                >
                  <div
                    ...
                    title={tr(`长度 ${len}: ${count} 条`, `Length ${len}: ${count} paths`, `Độ dài ${len}: ${count} đường dẫn`)}
                  />
