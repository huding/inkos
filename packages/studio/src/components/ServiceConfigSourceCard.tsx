...
export function ServiceConfigSourceCard({ onChange }: { onChange?: () => void }) {
  ...
  const load = async () => {
    try {
      ...
      setError(e instanceof Error ? e.message : tr("读取配置来源失败", "Failed to load config source", "Không thể tải nguồn cấu hình"));
    }
  };
  ...
  const importEnvConfig = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : tr("导入环境变量配置失败", "Failed to import env config", "Không thể nhập cấu hình env"));
    } finally {
      setImporting(false);
    }
  };
  ...
  if (!data && !error) {
    return (
      <div className="rounded-xl border border-border/40 bg-card/70 p-4 text-sm text-muted-foreground/70">
        {tr("正在读取配置来源…", "Loading config source…", "Đang tải nguồn cấu hình…")}
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="rounded-xl border border-amber-500/30 bg-amber-500/[0.04] p-4 text-sm text-amber-600">
        {error ?? tr("读取配置来源失败", "Failed to load config source", "Không thể tải nguồn cấu hình")}
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-muted-foreground/70 mt-1">
            {tr("Studio 运行时：", "Studio runtime:", "Studio runtime:")}
      ...
      {storedConfigSource === "env" ? (
        <div className="rounded-lg border border-amber-500/25 bg-amber-500/[0.04] p-3 text-xs text-muted-foreground/80">
          {tr(
            ...
            "Một cài đặt cũ đánh dấu `.env` là ưu tiên. Studio runtime bỏ qua nó; CLI, daemon và môi trường triển khai vẫn có thể dùng lớp ghi đè env.",
          )}
        </div>
      ) : null}
      ...
      {envDetected ? (
        <div className="rounded-lg border border-amber-500/25 bg-amber-500/[0.04] p-3 text-xs text-muted-foreground/80 space-y-1.5">
          <div className="text-foreground">
            {tr("检测到 LLM 环境变量覆盖：", "Detected LLM environment variable override:", "Đã phát hiện ghi đè biến môi trường LLM:")}
          ...
          <div className="text-muted-foreground/70 pt-1">
            {tr(
              ...
              "Đã phát hiện .env nhưng Studio và agent không dùng trực tiếp. Nhấn 'Nhập cấu hình đã phát hiện' để lưu thành cấu hình dịch vụ Studio.",
            )}
          </div>
        </div>
      ) : (
        <div className="rounded-lg border border-border/30 bg-secondary/20 p-3 text-xs text-muted-foreground/75">
          {tr(
            ...
            "Không phát hiện biến ghi đè LLM nào trong .env dự án hoặc toàn cục. Cấu hình dự án và dịch vụ Studio được dùng trực tiếp.",
          )}
        </div>
      )}
