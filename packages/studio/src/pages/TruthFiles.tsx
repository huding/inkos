          ...
          {selected && fileData?.content != null ? (
            <>
              {isLegacyShim && (
                ...
                >
                  <div className="font-medium">{tr("兼容层只读 / Read-only compat shim", "Read-only compat shim", "Tầng tương thích chỉ đọc")}</div>
                  <div className="mt-1">
                    {tr("本文件已废弃，仅供外部读取。权威来源：", "This file is deprecated and read-only. Authoritative source:", "Tệp này đã lỗi thời, chỉ dùng để đọc ngoài. Nguồn chính thống:")}
              ...
              {isRuntimeDiagnostic && (
                ...
                >
                  <div className="font-medium">{tr("运行时诊断文件 / Runtime diagnostic", "Runtime diagnostic", "Tệp chẩn đoán runtime")}</div>
                  <div className="mt-1">
                    {tr("这里展示本章写作时的上下文选择、保护层、可压缩层和预算 trace。它只用于追溯系统看了什么，不作为可编辑设定。", "Shows context selection, protection layers, compressible layers, and budget trace used when writing this chapter. For tracing what the system saw — not an editable setting.", "Hiển thị lựa chọn ngữ cảnh, lớp bảo vệ, lớp có thể nén và trace ngân sách khi viết chương này. Chỉ để truy vết hệ thống xem gì — không phải cài đặt có thể chỉnh sửa.")}
                  </div>
                </div>
              )}
