        ...
        {tab === "canon" && (
          <>
            <p className="text-sm text-muted-foreground">
              {tr("母本可以来自已有 InkOS 书籍，也可以直接上传外部 TXT、Markdown 或 PDF 小说。", "Use an existing InkOS book or upload an external TXT, Markdown, or PDF novel as canon.", "Sử dụng sách InkOS có sẵn hoặc tải lên tiểu thuyết TXT, Markdown hoặc PDF bên ngoài làm mẫu gốc.")}
            </p>
            <div className="inline-flex rounded-lg border border-border bg-secondary/20 p-1">
              {(["book", "file"] as const).map((sourceType) => (
                ...
                >
                  {sourceType === "book" ? tr("已有书籍", "Existing book", "Sách hiện có") : tr("上传外部母本", "Upload external canon", "Tải lên mẫu gốc bên ngoài")}
                </button>
              ))}
            </div>
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-3">
              <select value={ffMode} onChange={(e) => setFfMode(e.target.value)}
                ...
                <option value="canon">{tr("原著向", "Canon-compliant", "Bám sát nguyên tác")}</option>
                <option value="au">{tr("架空 AU", "Alternate Universe (AU)", "Vũ trụ thay thế (AU)")}</option>
