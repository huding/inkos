    ...
    <div className="max-w-xl mx-auto space-y-6">
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-center gap-2">
          ...
          {status.state === "connected" && (
            <span className="text-xs text-emerald-500">
              {tr(`连接成功，${models.length} 个模型`, `Connected, ${models.length} models`, `Kết nối thành công, ${models.length} mô hình`)}
              {detectedModel
                ? tr(
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label={tr("流式响应", "Streaming", "Phản hồi luồng")}>
        ...
        <div className="space-y-2">
          <p className="text-xs text-muted-foreground/70 font-medium uppercase tracking-wider">
            {tr(`模型目录（${models.length}）`, `Model catalog (${models.length})`, `Danh mục mô hình (${models.length})`)}
          </p>
          <div className="flex gap-2">
            <input
              ...
              placeholder={tr("输入模型 ID，例如 gemini-3.1-pro", "Enter a model ID, e.g. gemini-3.1-pro", "Nhập ID mô hình, ví dụ gemini-3.1-pro")}
          ...
          <p className="text-xs text-muted-foreground/60">
            {tr("测试连接发现的模型和手动添加的模型都会在保存后持久化；内置目录只作为兜底。", "Discovered and manually added models are persisted on save; the built-in catalog is only a fallback.", "Các mô hình được phát hiện và thêm thủ công sẽ được lưu khi lưu; danh mục tích hợp chỉ là dự phòng.")}
          </p>
          {hasModelCatalog && (
          <div className="space-y-2">
            {models.length > 0 ? (
                ...
                {models.map((m) => (
                  <span key={m.id} className="inline-flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-md bg-emerald-500/[0.06] text-emerald-600 dark:text-emerald-400 border border-emerald-500/15">
                    ...
                    <button
                      ...
                      aria-label={tr(`移除模型 ${m.id}`, `Remove model ${m.id}`, `Xóa mô hình ${m.id}`)}
        ...
        <details className="group pt-2 border-t border-border/20">
          <summary className="text-xs text-muted-foreground/60 cursor-pointer select-none hover:text-muted-foreground transition-colors py-2">
            {tr("高级参数", "Advanced", "Thông số nâng cao")}
          </summary>
