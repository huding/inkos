...
export function ProjectSettings({ nav, theme, t }: { nav: Nav; theme: Theme; t: TFunction }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <SettingsCard
        title={"Agent Skills"}
        description={tr("导入标准 SKILL.md 专业能力包。Chat 可以按意图自主使用，也可以在输入框用 + 号强制启用。", "Import standard SKILL.md expertise packages. Chat can choose a skill from intent, or you can force one from the + menu.", "Nhập các gói kỹ năng SKILL.md chuẩn. Chat có thể tự chọn skill theo ý định, hoặc bạn có thể buộc dùng từ menu +.")}
        icon={<Bot size={18} />}
      >
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/60 bg-secondary/20 p-3">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {tr("兼容 AgentSkills / OpenClaw：选择包含 SKILL.md 的完整文件夹，静态参考资料会一并导入；脚本不会自动执行。", "AgentSkills / OpenClaw compatible: select a complete folder containing SKILL.md. Static references are imported; scripts are never auto-executed.", "Tương thích AgentSkills / OpenClaw: chọn thư mục đầy đủ chứa SKILL.md. Tài liệu tham chiếu tĩnh được nhập; script không bao giờ tự chạy.")}
              </p>
            </div>
            <button
              type="button"
      ...
      <SettingsCard
        ...
        description={tr("集中查看和调整内置提示词。修改会保存为项目级覆盖文件，不会改动内置默认值。", "Review and tune built-in prompt packs. Edits are saved as project overrides without changing the defaults.", "Xem và chỉnh sửa các gói prompt tích hợp. Chỉnh sửa được lưu thành tệp ghi đè dự án, không thay đổi mặc định tích hợp.")}
          ...
          <div className="rounded-xl border border-border/60 bg-secondary/20 p-3">
            {selectedPrompt ? (
              <div className="space-y-3">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">
                      {tr("当前来源", "Source", "Nguồn hiện tại")}: {selectedPrompt.source}
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      }, tr("提示词已恢复默认", "Prompt reset to default", "Đã khôi phục mặc định prompt"))}
                ...
                <details className="rounded-xl border border-border/50 bg-background/50 p-3">
                  <summary className="cursor-pointer text-sm font-semibold text-muted-foreground">
                    {tr("查看内置默认", "View built-in default", "Xem mặc định tích hợp")}
                  </summary>
              ...
              <p className="text-sm text-muted-foreground">
                {tr("选择左侧提示词后编辑。", "Select a prompt on the left to edit it.", "Chọn prompt ở bên trái để chỉnh sửa.")}
              </p>
            )}
          </div>
        </div>
      ...
      <SettingsCard
        title={tr("联网研究搜索服务", "Research Search Provider", "Nhà cung cấp tìm kiếm nghiên cứu")}
        description={tr("给 research_web 配置外部搜索 API。未配置时仍可用服务器环境变量 TAVILY_API_KEY 作为兜底。", "Configure the external search API used by research_web. If unset, the server may still use TAVILY_API_KEY as a fallback.", "Cấu hình API tìm kiếm bên ngoài dùng bởi research_web. Nếu không đặt, server vẫn có thể dùng TAVILY_API_KEY làm fallback.")}
        icon={<Search size={18} />}
      >
        <label className="flex items-center gap-2 text-sm">
          <input
        ...
        <Collapse open={researchSearch.enabled}>
          <div className="grid gap-2 pt-1 md:grid-cols-2">
            <label className="space-y-1 text-xs text-muted-foreground">
              <span>{tr("搜索服务", "Provider", "Dịch vụ tìm kiếm")}</span>
              <select
            ...
            <label className="space-y-1 text-xs text-muted-foreground md:col-span-2">
              <span>{tr("Base URL（可选，自定义兼容端点）", "Base URL (optional custom compatible endpoint)", "Base URL (tùy chọn, điểm cuối tương thích tùy chỉnh)")}</span>
              <input
            ...
            <label className="space-y-1 text-xs text-muted-foreground md:col-span-2">
              <span>{tr("API Key（可选；留空则读环境变量）", "API key (optional; leave blank to use env var)", "API Key (tùy chọn; để trống để đọc biến môi trường)")}</span>
              <input
                value={researchSearch.apiKey}
                onChange={(e) => setResearchSearch((prev) => ({ ...prev, apiKey: e.target.value }))}
                type="password"
                placeholder={tr("可直接填 key，或只填环境变量名", "Paste key, or use env var only", "Có thể điền key trực tiếp, hoặc chỉ điền tên biến môi trường")}
                className={`${fieldClass} font-mono`}
              />
            </label>
          </div>
            ...
            <div key={i} className="rounded-xl border border-border/60 bg-secondary/20 p-3 space-y-2">
              ...
              {ch.type === "webhook" && (
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <input value={ch.secret ?? ""} onChange={(e) => updateChannel(i, { secret: e.target.value })} placeholder={tr("secret (可选)", "secret (optional)", "secret (tùy chọn)")} className={`${fieldClass} font-mono`} />
                </div>
              )}
            </div>
          ))}
