...
function CoverConfigCard() {
  ...
  useEffect(() => {
    ...
    void fetchJson<CoverConfigPayload>("/cover/config")
      ...
      .catch((error) => {
        if (cancelled) return;
        setStatus("error");
        setMessage(error instanceof Error ? error.message : tr("读取封面配置失败", "Failed to load cover config", "Không thể tải cấu hình bìa"));
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setStatus("error");
      setMessage(error instanceof Error ? error.message : tr("保存封面配置失败", "Failed to save cover config", "Không thể lưu cấu hình bìa"));
    }
  };
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground/70">
            {tr(
              ...
              "Chỉ cấu hình nhà cung cấp và mô hình bìa; kích thước bìa do gợi ý bìa truyện ngắn và mặc định nội bộ xử lý.",
            )}
          </p>
        </div>
        {selected?.connected && (
      ...
      <label className="space-y-1.5">
        ...
        <span className="block text-[11px] leading-5 text-muted-foreground/55">
          {tr(
            ...
            "Để trống để dùng địa chỉ mặc định của nhà cung cấp; giá trị tùy chỉnh sẽ là gốc API tạo bìa.",
          )}
        </span>
      </label>
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder={tr("搜索服务商", "Search providers", "Tìm kiếm nhà cung cấp")}
