...
const PHASE_LABELS: Record<Phase, { zh: string; en: string; vi: string }> = {
  ...
  validate: { zh: "校验", en: "Validate", vi: "Xác thực" },
};
...
const PHASE_SUBVIEWS: Record<Phase, ReadonlyArray<{ key: string; zh: string; en: string; vi: string }>> = {
  world: [
    { key: "chat", zh: "对话", en: "Chat", vi: "Hội thoại" },
    { key: "anchor", zh: "世界锚点", en: "World anchor", vi: "Mỏ neo thế giới" },
  ],
  scale: [],
  structure: [
    ...
    return (
      <div className={`p-6 text-sm ${c.muted}`}>
        {tr(
          ...
          "Chưa có mỏ neo thế giới. Hãy chuyển sang “Hội thoại” và nhờ AI xây dựng thế giới và nhân vật.",
        )}
      </div>
    );
  }
  ...
  return (
    <div className={`p-6 text-sm ${c.muted}`} data-testid="film-scale-placeholder">
      {tr(
        ...
        "Cấu hình quy mô (tính năng P2) — đặt mục tiêu số nút, độ sâu phân nhánh, số kết cục và các tham số khác tại đây.",
      )}
    </div>
  );
}
