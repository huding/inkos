...
function assertScriptDeliverable(script: string, language: "zh" | "en" | "vi"): void {
  const characterHeadings = language === "en" ? ["Characters"] : language === "vi" ? ["Nhân vật", "Characters"] : ["人物", "Characters"];
  const scriptHeadings = language === "en" ? ["Script"] : language === "vi" ? ["Kịch bản", "Script"] : ["剧本正文", "Script"];
...
function mergeRequirements(
  ...
  const extraLabel = language === "zh" ? "补充要求：" : language === "vi" ? "Yêu cầu bổ sung:" : "Additional requirements:";
  return [
    instruction.trim(),
    requirements?.trim() ? `\n${extraLabel}\n${requirements.trim()}` : "",
  ].filter(Boolean).join("\n");
