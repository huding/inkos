...
- **铁律**：不要把“再次提到”“换个说法重述”“抽象复盘”当成推进。只有状态真的变了，才更新最近推进。只是出现过的旧 hook，放进 mention 数组。`;
  ...
  const langPrefix = resolvedLang === "vi"
    ? `【LANGUAGE OVERRIDE】ALL output (state card, hooks, summaries, subplots, emotional arcs, character matrix) MUST be in Vietnamese (Tiếng Việt). The === TAG === markers remain unchanged.\n\n`
    : isNonCjk
