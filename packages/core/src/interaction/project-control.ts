  ...
  try {
    ...
    const failedSession = appendInteractionEvent({
      ...
      currentExecution: {
        ...
        stageLabel: localizedRequest.language === "zh" ? `执行失败：${localizedRequest.intent}` : localizedRequest.language === "vi" ? `Thất bại: ${localizedRequest.intent}` : `failed ${localizedRequest.intent}`,
      },
    }, {
      kind: "task.failed",
      timestamp: Date.now(),
...
async function detectProjectInteractionLanguage(projectRoot: string): Promise<"zh" | "en" | "vi" | undefined> {
  try {
    ...
    return parsed.language === "en" ? "en" : parsed.language === "zh" ? "zh" : parsed.language === "vi" ? "vi" : undefined;
  } catch {
    return undefined;
  }
}
